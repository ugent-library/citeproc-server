...
// POST /cites
function multiStyleRequest(request, response) {
    ...
    request.on("end", function() {
        let requestConfig = {};
        let preparedData, postObj;
        let styleUrlObjects = [];

        Promise.resolve().then(function(){

            // Parse url from request object, and merge it with default config
            requestConfig = _.extend({}, defaultRequestConfig);
            request.currentURI.searchParams.forEach(function(val, key){
                if(paramHasMultipleValues[key]){
                    if(!(Array.isArray(requestConfig[key]))){
                        requestConfig[key] = [val];
                    }
                    else {
                        requestConfig[key].push(val);
                    }
                }
                else {
                    requestConfig[key] = val
            ...
            if(Array.isArray(requestConfig.style)){
                /*
                    two or more engines of the same
                    type generate a race condition,
                    and will render the engine unusable
                    the next request as "working" remains "true"
                */
                styles = Array.from(new Set(requestConfig.style))
            } else {
                styles = [requestConfig.style]
            }
            ...
            for(let i = 0;i < styles.length;i++) {
                let style = styles[i]
                let ce = cslLoader.resolveStyle(style)
                    .then(function(resolvedUrlObj){
                        styleUrlObjects.push(resolvedUrlObj)
                        return engineCache.getEngine(resolvedUrlObj, requestConfig.locale);
                    })
                    .then(function(engine){
                        engine.style = style;
                        return engine;
                    })
                citeprocEngines.push(ce);
            }

            return citeprocEngines;

        }).then(function(citeprocEngines){

            let responseBody = [];

            return Promise.all(citeprocEngines).then(function(values){
                for(let i = 0;i < values.length;i++){

                    let citeprocEngine = values[i];
                    let responseItem = {
                        style: citeprocEngine.style,
                    };
                    responseBody.push(responseItem);

                    citeprocEngine.cslEngine.sys.items = preparedData.reqItemsObj;
                    citeprocEngine.cslEngine.setOutputFormat("html");
                    ...
                    if (requestConfig.bibliography == "1") {
                        responseItem.bibliography = citeprocEngine.cslEngine.makeBibliography();
                    }
                    if (requestConfig.citations == "1") {
                        ...
                        responseItem.citations = citations;
                    }

                    //reset citeproc engine before saving
                    engineCache.returnEngine(styleUrlObjects[i], requestConfig.locale, citeprocEngine);
            ...
            .then(function(){
                ...
                response.write(JSON.stringify(responseBody), "utf8");
                response.end();
            })
...
var server = http.createServer(function (request, response) {
    ...
    request.currentURI = new URL(request.url, "http://localhost:"+config.port);//host not important
    if (request.currentURI.pathname == "/cites") {
        multiStyleRequest(request, response)
        return
    }
